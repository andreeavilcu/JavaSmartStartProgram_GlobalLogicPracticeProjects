...
public class CustomerTest {
    ...
    void testUpdateCustomer_InvalidData(){
        Customer c = new Customer(6, "Eve", "123", "Madrid");
        service.addCustomer(c);

        Customer invalidUpdate = new Customer(6, "", "123", "");
        try {
            service.updateCustomer(invalidUpdate);

        } catch (InvalidCustomerException e) {
            assertEquals("Customer name and address cannot be empty", e.getMessage());
        }
    }
