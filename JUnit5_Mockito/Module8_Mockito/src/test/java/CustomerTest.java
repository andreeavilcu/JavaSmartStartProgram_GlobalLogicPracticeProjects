...
public class CustomerTest {
    ...
    @TestFactory
    Collection<DynamicTest> generateDynamicTestForCustomer(){
        return Arrays.asList("Alice", "Bob", "Charlie")
                .stream()
                .map(name -> dynamicTestForCustomer(name))
                .collect(Collectors.toList());
    }

    private DynamicTest dynamicTestForCustomer(String name){
        return DynamicTest.dynamicTest("Test that customer name '" + name + "' is not empty",
                () -> {
                    Customer customer = new Customer(200, name, "123456789", "Berlin");
                    service.addCustomer(customer);

                    Customer retrieved = service.getCustomerById(200);
                    assertNotNull(retrieved);
                    assertFalse(retrieved.getCustomerName().isEmpty());
                });
    }

    @ParameterizedTest
    @ValueSource(strings = {"Alice", "Bob", "Charlie"})
    void dynamicTestCustomerNameIsNotEmpty(String name){
        Customer customer = new Customer(300, name, "111111", "Berlin");
        service.addCustomer(customer);

        Customer retrieved = service.getCustomerById(300);
        assertNotNull(retrieved);
        assertFalse(retrieved.getCustomerName().isEmpty(), "Customer name should not be empty");
    }
}
